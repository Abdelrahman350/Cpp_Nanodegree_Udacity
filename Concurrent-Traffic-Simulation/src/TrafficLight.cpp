...
T MessageQueue<T>::receive()
{
    ...
    // read queue vector under mutex lock
...
void MessageQueue<T>::send(T &&message)
{
    ...
    // send queue under lock guard for the mutex
...
void TrafficLight::cycleThroughPhases()
{
    ...
    std::mt19937 rng(device()); // Random number generator
    std::uniform_int_distribution<std::mt19937::result_type> distribution(4000, 6000); // dist betweeen 4000ms and 6000ms
    int CycleDuration = distribution(rng); // duration of a single simulation cycle in ms

    while (true) 
    {
        // wait 1ms between two cycles
        std::this_thread::sleep_for(std::chrono::milliseconds(1) + std::chrono::milliseconds(CycleDuration));
        ...
        CycleDuration = distribution(rng); // Update cycle duration
    }
